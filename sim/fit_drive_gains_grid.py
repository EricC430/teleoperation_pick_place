...
Grid variant (2026-09-18 merge): the whole (tracking_error x damping_fraction) grid as parallel envs
in ONE scene, fed by `traj.npz`. `fit_drive_gains.py` is the one-combo-per-process variant that reads
parquet directly and has `--sign-override`. Both use `joint_mapping.py`; neither has been run on the
corrected unit conversion yet.
...
stiff even when its error against state is small.
    ...
    ./sim/run_in_container.sh fit_drive_gains_grid.py --headless \
...
parser.add_argument("--out", default="/workspace/test_isaaclab/omx_sim/out/fit_drive_gains_grid")
...
out_path = os.path.join(args.out, "fit_drive_gains_grid.json")
