...
def main(root: Path) -> int:
    ...
    # ---- 檢查 2：每台相機的影片總幀數 vs parquet 總行數 ----
    #
    # 🔴 2026-09-18 修正。原本「以檔名配對影片與 parquet、逐檔比對」只在 LeRobot v2.x（一集一個檔）
    ...
    #
    # ⚠️ 已知弱點（誠實記下）：比的是「每台相機的總和」，不是逐 episode。若某集多 1 幀、另一集少 1 幀，
    # 總和會互相抵銷而漏抓。本腳本要防的主要失效（info.json 宣稱總數 ≠ 影片實際總數 → 訓練中途
    # IndexError，見 docstring）仍然抓得到；逐 episode 的精確比對要用 meta/episodes 的
    # from/to_timestamp 解碼計數，成本高得多，目前沒做。
    print("\n" + "─" * 60)
    print("檢查 2：每台相機的影片總幀數 vs parquet 總行數")
