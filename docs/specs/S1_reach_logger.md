...
| `a` | 記一個 **azimuth-limit**：方位角掃到邊界時按。**備註欄要註明是哪一種邊界**（見下） |
| `r` | 記 **reference**：墊子已對齊底座，輸入手臂目前指向的墊子角度 |
| `q` | 存檔離開 |

> 🔴 **`shoulder_pan`（方位角）的可用範圍由「哪個限制先卡住」決定，按 `a` 時備註要註明是哪一種：**
>
> | 備註範例 | 意思 |
> |---|---|
> | `arm hits camera mount, left` / `..., right` | 手臂轉過去會**實體撞到第三視角相機/支架** —— **2026-08-31 現場就是這個**，量到 ≈ 135° 扇區（`theta ∈ [−90°, +45°]`）。硬機械限制，非相機視野 |
> | `cable, left` / `cable, right` | 改走線後傳輸線的實際卡點（2026-08-31 未在任務區造成限制） |
> | `cam FOV, ... @ start pose` | 物體在起始姿態相機畫面出框時的 `shoulder_pan` 角（視覺限制，可調相機角度時才會是它） |
>
> 相機/支架是實體障礙、肉眼看得到 → 轉到快撞前停、按 `a`。餵 S2 的扇區取所有來源裡**最窄**的。
> 凍結前寧可扇區取窄——之後不能再放寬（同 `r_outer` 的 margin 邏輯）。
> ⚠️ 撞擊界綁定「現在的相機架設位置」，相機一移要重量。
