...
| S4 | `S4_sim_teleop_collect.md` | `scripts/sim_teleop_collect.py` | 🟡 要 leader 臂，不要 follower | ✅ **2026-09-03 解除封鎖（D029）**：模擬器＝Isaac Sim、D025 前提 3 撤銷、leader 直插實驗室 5090 Linux 筆電。**先做 §2-2 的 USD 稽核，再做 §4 管線** |
| S5 | `S5_sim_replay_augmentation.md` | `scripts/sim_replay_augment.py` | ❌ 不用（重播既有真實錄製，非即時） | 🟡 **2026-09-15 提議中，尚未裁決**（`[AI提議]`）。**2026-09-18 四個缺口的準備工具已建（§11），模擬端腳本尚未在 5090 上跑過。**技術上無硬阻塞——§2 的四個缺口沒有一個卡住開工，只有缺口 4（相機幾何）需要 lab day 且可延後/外包 |
