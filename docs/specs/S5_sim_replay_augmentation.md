...
### 🔴 2026-09-21 §2-C：抓取對不上的根因是**關節零位偏移**，只有實機能量

`[Eric說 2026-09-21]`：**手臂底座高於桌面 15 cm**。這個數字本身沒問題，但**單獨套上去會讓落差惡化**
（夾爪從高出物體 11.2 cm 變成 26.2 cm），因為用真實關節角重播時，夾爪落在自己基座**上方** 15.3 cm——
若基座又高於桌面 15 cm，抓取瞬間夾爪就在桌面上方 30 cm，而同一時刻真實影片裡它正夾著桌上約 9 cm 高的紙杯。
**兩者不可能同時成立 → 有一項沒被建模。**

| 假設 | 檢驗 | 結果 |
|---|---|---|
| 符號接反（`elbow_flex`） | 翻符號後與擺放點的 x 相關性 | **+0.633 → −0.337（反相關）→ 排除** |
| 關節零位偏移 | 對 60 集擬合固定偏移 | 解出抓取高度 8.2 cm（夾爪在基座**下方** 6.8 cm）——正是伸手下探的形狀 |

**偏移假設兼容所有證據**：常數偏移不改變「隨擺放點變化」的相關性（所以 r=+0.63 保持住），卻會造成
系統性的絕對位置誤差（那個從頭到尾都在、一直被我歸錯因的 +10 cm dx）。成因很正常：**lerobot 校正檔的
「0 度」與 URDF 的零位姿是兩回事，從來沒有人對過。**

🔴 **但擬合出來的數值不可用，我不會把它寫進常數**：`shoulder_lift`／`elbow_flex`／`wrist_flex` 共平面、
繞同一軸，偏移可互相抵換（擬合給出 elbow +32.5°、wrist_flex +32.4°，那是退化在說話，不是量測），
且水平仍殘留 9.3 cm。**從錄好的資料解不開這個退化。**
...
`[Eric說 2026-09-21]`（看 `outputs/videos/ep0_real_vs_sim.mp4` 後指出）：
**「手臂並沒有放到藍色 15cm 檯子上面，方位也不對」。** 成立。`omx_scene_cfg.py` 原本把
`ARM_BASE_POS` 直接放在桌面（z = `TABLE_TOP_Z`），完全沒有把檯子模型化；場景裡唯一的藍色方塊是
標為 PLACEHOLDER 的**目標垃圾桶**，擺在 `(0.10, 0.30)`，跟手臂無關。

已修：新增 `scene_constants.ARM_RISER_HEIGHT = 0.15`（高度是 Eric 量的；**footprint 仍是 PLACEHOLDER**），
`ARM_BASE_POS` 改成 `TABLE_TOP_Z + ARM_RISER_HEIGHT`，並在 pan 軸正下方加上檯子本體。

🔴 **修正後垂直落差從 11.2 cm 變成約 26 cm——變大了，而這正是預期結果。**
先前 §2-C 的算式就預測過：基座抬高 15 cm 會讓落差惡化，除非同時存在關節零位偏移。
**所以這不是退步，是把兩個未知數收斂成一個**：基座高度已知且已修，剩下的全部落在關節零位偏移上，
沒有別的地方可以藏。這也讓 §2-C 的結論更強，而不是更弱。

**從真實影片另外看到、規格先前沒記的兩件事：**
- **真實紙杯是立著的**（杯口朝上，約 9 cm 高），模擬物體是**躺倒的罐子**（中心離桌面 4.1 cm）。
  這獨立造成約 4–5 cm 的抓取高度差，跟基座那 15 cm 是兩回事。Eric 說要重建資產時，**杯子的姿態要一起定**。
- **front-left 視野裡看不到獨立的目標容器**，但任務字串是 "place it into the box"。
  現在場景裡有兩個藍色方塊（新的檯子＋舊的 bin placeholder）。🟡 **待確認：真實場景到底有沒有目標容器？**
  沒有的話那個 bin 要拿掉——不要在畫面裡放一個現實不存在的東西，那正是 DR 資料最不該有的東西。

🟡 **「方位也不對」尚未處理**——需要 Eric 指明是「手臂朝向（yaw）」還是「檯子位置」。
⚠️ 注意基座 yaw 與 `shoulder_pan` 零位偏移在數學上等價、無法分離，但對模擬只要總和正確即可。

### 🔴 2026-09-21 §2-E：場景幾何從「暫時可接受」升級為「直接污染產品」

`[Eric說 2026-09-21]`，逐項回覆：
- **需要一個桌面上 15 cm 高的台子，手臂底座和垃圾桶都放在上面**（不是兩個分開的東西）。已改。
- **方位**：影片裡模擬手臂面向左下（向內）；實際上**檯子在桌子右下角邊緣，手臂在其上朝左上（向外）**。
  桌子已改成往 +X／+Y 延伸，讓手臂位於近側右角。
- **杯子確實立著**。已記錄（模擬物體仍是躺倒的罐子，待資產重建）。
- 🔴 **「即使兩隻手臂同時啟動，手腕關節和一些關節的轉動跟影片不一致」** ← 見下。

**關於方位，有一點要先講清楚，否則會有錯誤預期：修好方位不會讓抓取變準。**
擺放點座標定義在**手臂自己的座標系**（墊子 0° ＝手臂正前方，`x_pan_cm`/`y_pan_cm` 相對 pan 軸），
所以手臂轉向時擺放點跟著轉——**手臂與物體的相對幾何不受基座 yaw 影響**。方位影響的是畫面，不是抓取。
而且影片裡看到的朝向有一部分是 PLACEHOLDER 相機造成的，用它判斷朝向本身就不可靠。

**「關節轉動不一致」的資料層線索（`[已查證 2026-09-21]`，全 60 集）：**

| joint | ep0 活動範圍 | 60 集中位數 |
|---|---:|---:|
| shoulder_lift / elbow_flex | 99° / 101° | 94.5° / 77.1° |
| shoulder_pan / wrist_flex | 50° / 28° | 53.5° / 32.6° |
| **wrist_roll** | 56.8° | **10.1°** |
| **gripper** | **9.8°** | **9.7°** |

**夾爪在整個資料集只變動約 10 個單位（47~60）。** 模擬把它當「度」直接餵給 `gripper_joint_1`，
所以模擬夾爪只轉 10°，再加上 mimic 幅度只有一半（缺口 2 殘留），第二指只轉 5°——**畫面上幾乎看不出開合**，
而真實影片裡夾爪明顯開合。**這一路不是純偏移，是比例（scale）不對：記錄單位 ≠ `gripper_joint_1` 的度數。**
`wrist_roll` 有同樣嫌疑（多數集只動 10°）。
→ **五姿態測試因此要「每關節兩個姿態」**：一個姿態只解得出偏移，兩個才能分辨偏移與比例。

🔴 **升級：S5 的產品就是畫面本身，所以場景幾何的每個 PLACEHOLDER 都直接污染產品。**
檯子的長寬與中心、垃圾桶在檯上的位置、桌子尺寸與手臂在哪一角——**目前全是 AI 編的數字**
（見 `scene_constants.py` 標示）。先前這可接受，因為只是驗管線；現在要產真資料就不可接受。
缺口 4 的範圍因此從「相機內外參」擴大為「整個場景幾何」，lab day 清單同步擴充（見 §2-E 上方對話紀錄）。
**在量到之前，再調這些數字只是拿猜的去修猜的。**

### 🟡 2026-09-21 §2-F：場景與相機已全部改用量測值，手臂消失變成偏移的指示器

`[Eric說 2026-09-21]` 補齊了第三視角相機：手臂底座左緣再往左 20 cm、**檯子前緣向內（−X）4.5 cm**、
高 11 cm、由右側水平線往中心 45°、**俯角約 10°**。（第一版我把「向內」讀成 +X，渲染出來相機落在
擺放點雲裡、手臂還跑到相機後方——用平面圖一眼看出來，不是靠渲染猜的。）

**新增 `scripts/plot_scene_layout.py`：由 `scene_constants.py` 直接畫俯視平面圖。**
渲染會把「幾何錯」和「取景錯」混在一起，平面圖只看幾何。**流程改成：改場景數字 → 先出平面圖 →
確認合理才渲染**，一次省下約 4 分鐘的渲染。

修正後（`outputs/renders/replay_ep0_v4/`、`outputs/videos/ep0_real_vs_sim_v4.mp4`）：
畫面結構與真實影片一致——杯子在桌面中央、檯子在右側。

**2026-09-21 續：又三處修正（`[Eric說]`）後，畫面結構已與真實一致。**
(1) 相機的 11 cm 是**相對檯面**，不是桌面 → z = 15+11 = **26 cm**；
(2) 檯子要**延伸到涵蓋相機**（左緣 +0.085 → **+0.305**，尺寸 0.22×0.47 → **0.23×0.71 m**）；
(3) 底座與垃圾桶前緣要**貼齊**檯子前緣——這是 AI 的 bug：餘裕原本加在四周，把前緣多推了 1 cm，
    改成只加在後側與遠側，三者前緣現在都是 0.0712。

結果（`outputs/renders/replay_ep0_v5/`、`outputs/videos/ep0_real_vs_sim_v5.mp4`）：
**手臂如預期出現在畫面裡了**，而且與真實影片結構對應——手臂都從右上方進入、杯子都在桌面中央、
檯子與支架都在右側。**唯一殘差就是模擬手臂不會下探**：夾爪高懸，真實則下到杯口。

🔴 **這正是把相機當成偏移偵測器的用法**：相機已經校正到結構吻合，殘差因此只剩關節零位偏移一個來源。

### 🟡 2026-09-21 §2-G：單位修正後重跑——25.8 → 14.9 cm，手臂終於下到杯子

合併 `origin/main` 後取得另一個 session 的兩項成果：**(a) `.pos` 的單位修正**（不是度，是
`RANGE_M100_100`，1 unit = 1.78–1.80 度）、**(b) S6 實機量到的關節零位**
（`calibration/2026-09-21_joint_zeros.csv`）。以此重跑 episode 0：

| | 修正前 | 修正後 |
|---|---|---|
| TCP 到物體最近距離 | 25.8 cm | **14.9 cm** |
| 渲染畫面 | 手臂高懸／出框 | **夾爪張開下探到杯子正上方** |

🔴 **同時要記下這是怎麼發現的，因為錯的是本 session 的判斷：**
我把 `.pos` 判定成「度」，依據是 `stats.json` 的數值範圍落在關節行程內——**但那個範圍同樣符合
`-100..100` 正規化，這個推論方法無法區分兩者，我卻把結論標成 `[已查證]`。** 正確做法是讀 lerobot
原始碼確認 `use_degrees` 預設為 False（另一個 session 做的）。**「範圍看起來合理」不是查證。**

**被這個錯誤污染、需要重跑的東西**（同 `joint_mapping.py` docstring 的警告）：
gap 1 的五組 drive gain 數字、`verify_grasp_attach` 的 ep0 結果、§2-A 的 SIGN 渲染對照
（是在 1.8 倍錯誤尺度下做的，`shoulder_lift=+1` 的結論很可能仍成立因為翻符號是粗差異，
但其餘五個關節「沒看到不合」的觀察要在正確尺度下重做）。

**不受影響、可直接沿用的**：管線本身、平面圖工具、對照組方法、S6 的兩姿態法
（本 session 獨立推導出「要兩姿態才能分離零位與比例」，與另一個 session 實際量測的做法一致）。

**剩下的 14.9 cm，候選原因（未分辨）**：TCP 目前取 `link6`/`link7` 的 body 原點，不是手指實際接觸點；
杯子是平均直徑的圓柱近似；相機與腕部安裝仍是 PLACEHOLDER；五個關節的 SIGN 尚未在正確尺度下驗證。

#### 🔴 2026-09-21 續：那 14.9 cm 已拆解成兩項，兩項都有解

用 `reach_logger/fk.py` ＋ 記錄的擺放位置在**本機**重算（不需 Isaac Sim），
以 episode 0、placement `t1`、杯心取杯高一半：

| TCP 取法 | 甲 目前校正 | 乙 `shoulder_lift` 不扣 20.14° |
|---|---|---|
| `link6`/`link7` body 原點（離 link5 2.95 cm）| **14.9 cm** | 6.8 cm |
| **實測指尖**（離 link5 **8 cm**，`[柏宇說 2026-09-21]`）| 10.4 cm | **3.6 cm** |

✅ **左上角 14.9 cm 與 Isaac Sim 渲染量到的數字完全一致**——本機 FK 模型可以拿來預測，
不必每次開模擬器。

1. **TCP 值 4.5 cm。** 本節上面列的第一個候選（「不是手指實際接觸點」）**已可關閉**：
   指尖在 link5 前方 8 cm，是量出來的。`grasp_attach.py` 的 TCP 應改用這個點。
2. **`shoulder_lift` 的 20.14° 值 8.1 cm**，即 `S6_joint_zero_calibration.md` §4-a 的
   🟡 待裁決（甲／乙）。

**兩項都套用 → 3.6 cm，低於 5 cm 抓取閘門，抓取會觸發。**

#### ✅ 2026-09-21 再續：兩項都在容器裡跑過了（4090），第 1 項成立，第 2 項要修正說法

`[產出物]` `~/isaaclab_volume/omx_sim/s5_tcp_fingertip{,_liftB}/manifest.json`。

1. **TCP 改實測指尖：`14.9 cm → 10.4 cm`，與本機 FK 預測的 10.4 cm 完全一致。**
   `grasp_attach.tcp_pose_w()` 在容器裡跑起來沒有報錯——上一個 commit 留的
   「⚠️ 未在容器內跑過，第一次跑要確認」**可以關閉**。
2. 🔴 **「兩項都套用 → 3.6 cm，抓取會觸發」這句要修正：實測是 5.5 cm，而且抓取沒有觸發。**
   差額的來源已經查清楚（是接觸，不是誤差），完整證據在
   `S6_joint_zero_calibration.md` §4-a 的「裁決方法已執行」小節，不在這裡複述。

⚠️ `[AI推論]` **附帶發現：5 cm 抓取閘門的定義本身可疑。** 它量的是 TCP 到**杯心**的距離，
但杯子平均半徑約 3.1 cm——夾爪真的貼住杯身時，TCP 到杯心本來就是 4–6 cm。
換句話說，閘門的通過條件比「碰得到杯子」更嚴。這屬於 §2 gap 3（`grasp_attach`）的設計問題，
**尚未處理**，也還不知道該改成量到杯面、還是放寬半徑。
⚠️ 第 2 項仍是 `[AI提議]`，**未裁決**；§4-a 記錄了它在代數上還有一塊說不通。
裁決方法不變：把 `shoulder_lift` 的 offset 加上 20.14° 重渲一次，看夾爪有沒有下到杯口。
