...
- **缺口 1：`sim/fit_drive_gains.py`。** 拿一集真實 `action` 開環驅動模擬 articulation，逐幀比對模擬
  ...
  **讀法，不要跳過中間這一步就下結論：**
  ...
  5. **仍然成立、沒有被上一點取消的部分：** 五姿態測試（S4 §5-1）依然是唯一能乾淨裁決 SIGN 的方法——
     運動學重播照樣要把真實度數換算成模擬弧度（`sim/joint_mapping.py`），**符號接反一樣會渲染出鏡像
     的手臂**，只是不再跟增益/扭矩混在一起。上面兩條獨立證據（effort_limit 診斷、FK＋真實軌跡方向）
     都指向 SIGN=+1，但都不是單關節隔離量測。
