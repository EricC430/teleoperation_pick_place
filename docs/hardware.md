...
| 物品 | 用途 | 取得 |
|---|---|---|
| **捲尺／直尺（≥50 cm）** | 量 r_outer / r_inner —— **A6 數字的關鍵**。正式量測改用 URDF+FK（D026），但 placo/URDF 備妥前，捲尺是 fallback，且今天的 lab day 一定用捲尺 | 文具店／五金行 |
...
2. **量數字**：受限方位角 ／ `r_outer`（top-down 可夾）＋ `r_outer_side`（供參）／ 可下爪 r_min ／ 基座原點＋0° 前向線 → 填進 `docs/experiment_spec.md` §3（33/43 是抓取進場角差別，不是環帶，D023 §2026-08-31；正式量測程序見 `docs/specs/S1_reach_logger.md`，D026）
